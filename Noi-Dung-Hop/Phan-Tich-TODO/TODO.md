...
#### Ghi chú hiện tại
- Đã từng thử một nhánh thiết kế/tạo skill PM thực tế, nhưng theo quyết định hiện tại của anh, toàn bộ skill, spec, output test và note chuẩn hóa liên quan đã được xóa để phân tích lại từ đầu.
- Khi bắt đầu lại, cần rà lại hệ tài liệu PM thực tế, xác định đúng các loại văn bản/artefact cần có, rồi mới thiết kế lại hệ skill từ nền tảng.
- Theo định hướng mới, skill hạ tầng cần ưu tiên đầu tiên là: `markdown-to-pdf`.
- Spec mới đã tạo tại: `./SPEC-MARKDOWN-TO-PDF-V2.md`
- Final spec để build đã chốt tại: `./FINAL-SPEC-MARKDOWN-TO-PDF.md`
- Đã build skill thật trong: `skills/PM/markdown-to-pdf/`
- Đã tạo file demo nguồn: `./DEMO-MARKDOWN-TO-PDF.md`
- Đã xuất file demo PDF đầu tiên: `./DEMO-MARKDOWN-TO-PDF.pdf`
- Đã build tiếp skill `requirement-clarifier` trong: `skills/PM/requirement-clarifier/`
- Đã tạo file demo input: `./DEMO-REQUIREMENT-INPUT.md`
- Đã tạo file output demo Markdown: `./DEMO-REQUIREMENT-CLARIFICATION.md`
- Đã xuất file output demo PDF: `./DEMO-REQUIREMENT-CLARIFICATION.pdf`
- Đã tiếp tục chuẩn hóa các skill còn lại theo cùng mức chất lượng:
  - `task-list-builder`
  - `risk-issue-tracker`
  - `client-contract-draft-generator`
- Theo quyết định hiện tại, chưa tạo demo cho các skill này; đang dừng ở mức skill sẵn dùng + có template chuẩn.
- Đã bắt đầu thiết kế skill báo cáo định kỳ mới:
  - `periodic-project-report-generator`
  - Spec chi tiết tại: `./SPEC-PERIODIC-PROJECT-REPORT-GENERATOR.md`
- Đã build skill trong: `skills/PM/periodic-project-report-generator/`
- Đã tạo demo Markdown: `./DEMO-PERIODIC-PROJECT-REPORT.md`
- Đã xuất demo PDF: `./DEMO-PERIODIC-PROJECT-REPORT.pdf`
- Đã tiếp tục chuẩn hóa skill kế tiếp: `project-plan-generator` (không tạo demo theo quyết định hiện tại)

#### Nhóm skill đề xuất ban đầu
- **Skill tài liệu PM**
  - Meeting Minutes từ ghi chú họp thô
  - PRD draft từ ý tưởng/yêu cầu ban đầu
  - BRD draft từ ghi chú nghiệp vụ
  - Action Items / checklist sau họp
  - Draft hợp đồng giữa công ty và khách hàng
- **Skill phân tích yêu cầu**
  - bóc tách yêu cầu
  - tạo user stories
  - gợi ý acceptance criteria
  - phát hiện điểm mơ hồ để hỏi lại anh
- **Skill quản lý tri thức dự án**
  - lưu cuộc họp vào Obsidian
  - cập nhật decision log
  - cập nhật project summary
  - tạo note tóm tắt từ hội thoại quan trọng
- **Skill điều phối công việc PM**
  - tổng hợp việc cần làm
  - tóm tắt tiến độ
  - nhắc pending / follow-up

#### Nguyên tắc thiết kế skill
- Skill không quá rộng.
- Mỗi skill phải có đầu vào rõ ràng.
- Mỗi skill phải có đầu ra rõ ràng.
- Ưu tiên skill nhỏ, dễ kiểm chứng trước.
- Cần xác định rõ loại tài liệu PM thực tế trước khi xây skill cụ thể.

#### Kết quả mong muốn
- Có danh sách skill lõi đúng với các artefact/tài liệu PM thực tế, rồi mới bắt đầu hiện thực hóa lại từ đầu.
